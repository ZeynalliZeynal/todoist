...
import { CloudUpload, Trash } from 'vercel-geist-icons';
                  ...
                  <div className="flex flex-col gap-2 items-center">
                    <Tooltip>
                      <TooltipTrigger
                        ...
                        disabled={isUploading}
                        {...getRootProps()}
                        className="size-20 focus:ring-offset-8 rounded-full overflow-hidden group relative before:absolute before:z-[1] before:inset-0 before:transition hover:before:bg-background-100/25 before:rounded-full"
                      >
                        <span
                          className={cn(
                            'absolute text-sm text-foreground inset-0 z-[2] center opacity-0 group-hover:opacity-100 transition-opacity',
                            isUploading && 'opacity-100',
                          )}
                        >
                          <span className="center bg-background-100/50 backdrop-blur-sm gap-1 size-8 rounded">
                            {isUploading ? (
                            ...
                            onClick={() =>
                              startDeleteTransition(async () => {
                                try {
                                  ...
                                  toast.error(
                                    (error as ServerResponse).message,
                                  );
                                }
                              })
                            }
